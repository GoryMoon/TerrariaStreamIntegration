...
namespace TerrariaStreamIntegration
{
    ...
    class WorldGen_UpdateWorld_Patch
    {
        private static void Prefix()
        {
            MyModServer.Instance.UpdateWorld();
        }
    }
    ...
    class WorldFile_LoadHeader_Patch
    {
        private static void Postfix()
        {
            MyModServer.Instance.DeathCounter.Load();
        }
    }
    ...
    class WorldFile_SaveWorldHeader_Patch
    {
        private static void Postfix()
        {
            MyModServer.Instance.DeathCounter.Save();
        }
    }
    ...
    class Player_DropTombstone_Patch
    {
        ...
        {
            MyModServer.Instance.DeathCounter.OnPlayerDeath(__instance);
        }
    }
    ...
    {
        private static readonly MethodInfo _handleNetMethod = SymbolExtensions.GetMethodInfo(() => MyModServer.HandleModPacket(0, 0, null));
    ...
    class MessageBuffer_GetData_Patch
    {
        ...
        private static readonly MethodInfo _handleNetMethod = SymbolExtensions.GetMethodInfo(() => MyModServer.HandleModPacket(null));
        private static readonly MethodInfo _hijack = SymbolExtensions.GetMethodInfo(() => MyModServer.Hijack(0, null));
        private static readonly MemberInfo _setPosition = AccessTools.Property(typeof(Stream), "Position").SetMethod;
        ...
        private static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, ILGenerator generator)
        {
            ...
            foreach (var instruction in instructions)
            {
                ...
                // Hijack net ids
                //if (instruction.opcode == OpCodes.Callvirt && (MemberInfo) instruction.operand == _setPosition)
                //{
                //    yield return instruction;
                //    yield return new CodeInstruction(OpCodes.Ldloc_0);
                //    yield return new CodeInstruction(OpCodes.Ldarg_0);
                //    yield return new CodeInstruction(OpCodes.Call, _hijack);
                //    continue;
                //}
